...
class Matrix(object):
    ...
    def trace(self):
        ...
        return sum([self[i][j] for i in range(self.h) for j in range(self.w) if i == j])
    ...
    def __mul__(self, other):
        ...
        for i in range(self.h):
            ...
            for j in range(other.w):
                row.append(sum(i*j for i,j in zip(self[i], transposeOther[j])))
            result.append(row)
    ...
    def __rmul__(self, other):
        ...
        if isinstance(other, numbers.Number):
            return Matrix([[other * self[i][j] for j in range(self.w)] for i in range(self.h)])
